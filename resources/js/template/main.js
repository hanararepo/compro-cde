...
(function ($) {
  ...
  // Run once, after slider.js has registered the slider startup callback.
  $(document).ready(function () {
    var preloader = document.querySelector(".preloader");
    if (!preloader) {
      if (window.prepareSliderForReveal) window.prepareSliderForReveal();
      if (window.startSliderAfterPreload) window.startSliderAfterPreload();
      document.dispatchEvent(new Event("antra:ready"));
      return;
    }

    var innerBars = Array.from(preloader.querySelectorAll(".inner-bar"));
    ...
    // Reveal the hero before fading the overlay so the page background never flashes.
    preloaderTL.call(function () {
      if (window.prepareSliderForReveal) {
    ...
    if (!reducedMotion && document.querySelector(".text-animation-effect")) {
      var splitText = new SplitType(".text-animation-effect", { types: "chars" });
      preloaderTL.from(splitText.chars, {
        duration: 1.2,
        y: 40,
        autoAlpha: 0,
        stagger: 0.05,
  ...
  $(document).ready(function () {
    ...
    function customCursor(viewSelector = ".antra-hover-view") {
      ...
      const hasViewTargets = !!document.querySelector(viewSelector);
      if (hasViewTargets) $("body").append('<div class="mt-cursor-view"></div>');
      ...
      function animate() {
        ...
        let unsettled = Math.abs(targetX - cursorX) + Math.abs(targetY - cursorY) > 0.1;
        if (!unsettled) {
          cursorX = targetX;
          cursorY = targetY;
        }
        ...
        cursor.css({
          "--tl-cx": `${cursorX.toFixed(2)}px`,
          "--tl-cy": `${cursorY.toFixed(2)}px`,
        });
        if (activeHeadingElement && activeHeadingOverlay) {
          const rect = activeHeadingOverlay.getBoundingClientRect();
          activeHeadingOverlay.style.setProperty("--lens-x", `${(cursorX - rect.left).toFixed(2)}px`);
          activeHeadingOverlay.style.setProperty("--lens-y", `${(cursorY - rect.top).toFixed(2)}px`);
        }
        if (viewing) {
          viewX += (targetX - viewX) * 0.18;
          viewY += (targetY - viewY) * 0.18;
          const viewUnsettled = Math.abs(targetX - viewX) + Math.abs(targetY - viewY) > 0.1;
          if (!viewUnsettled) {
            viewX = targetX;
            viewY = targetY;
          }
          viewButton.css({ "--tl-vx": `${viewX.toFixed(2)}px`, "--tl-vy": `${viewY.toFixed(2)}px` });
          unsettled = unsettled || viewUnsettled;
        }
      ...
      $(window).on("mousemove.cursor", function (e) {
        ...
        targetY = e.clientY;
        if (!positioned) {
          cursorX = viewX = targetX;
          cursorY = viewY = targetY;
          positioned = true;
      ...
      let activeHeadingElement = null;
      let activeHeadingOverlay = null;

      function setupHeadingLenses() {
        $(".cursor-effect").each(function () {
          const $heading = $(this);
          if ($heading.find(".cursor-lens-overlay").length) return;
          const isDark = $heading.hasClass("text-white") || $heading.closest(".white-content, .bg-dark-1, .bg-dark-2").length > 0;
          
          const overlay = document.createElement("div");
          overlay.className = "cursor-lens-overlay" + (isDark ? " is-dark-heading" : "");
          overlay.innerHTML = $heading.html();
          $(overlay).find(".cursor-lens-overlay").remove();
          $(overlay).find("*").each(function () {
            this.style.setProperty("color", isDark ? "#000000" : "#ffffff", "important");
            this.style.setProperty("-webkit-text-fill-color", isDark ? "#000000" : "#ffffff", "important");
          });
          
          $heading.css("position", "relative").append(overlay);
        });
      }
      setupHeadingLenses();
      ...
      $("body")
        .on("mouseenter.cursor", ".cursor-effect", function () {
          activeHeadingElement = this;
          activeHeadingOverlay = this.querySelector(".cursor-lens-overlay");
          if (activeHeadingOverlay) {
            const rect = activeHeadingOverlay.getBoundingClientRect();
            activeHeadingOverlay.style.setProperty("--lens-x", `${(cursorX - rect.left).toFixed(2)}px`);
            activeHeadingOverlay.style.setProperty("--lens-y", `${(cursorY - rect.top).toFixed(2)}px`);
          }
        ...
        .on("mouseleave.cursor", ".cursor-effect", function () {
          if (activeHeadingOverlay) {
            activeHeadingOverlay.style.setProperty("--lens-x", "-9999px");
            activeHeadingOverlay.style.setProperty("--lens-y", "-9999px");
          }
          activeHeadingElement = null;
          activeHeadingOverlay = null;
          cursor.css("opacity", "1");
        });
      ...
      function hideCursor() {
        ...
        positioned = false;
        viewing = false;
        cursor.css({ visibility: "hidden", opacity: "1" });
      ...
      const onVisibilityChange = () => { if (document.hidden) hideCursor(); };
    ...
    // Keep the original desktop cursor, including laptops with a touch display.
    // Actual mouse movement starts it; the loop stops once its position settles.
    gsap.matchMedia().add("(min-width: 992px)", () => {
      customCursor();
      return () => window.destroyCustomCursor?.();
    });
