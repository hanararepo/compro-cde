<x-layouts.admin title="Edit Gallery Video">
    <div class="max-w-3xl mx-auto space-y-6">

        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Video Galeri</h2>
                <p class="text-sm text-slate-500 mt-1">
                    {{ $galleryVideo->getTranslation('title', 'id', false) ?: $galleryVideo->getTranslation('title', 'en', false) ?: 'Video Galeri' }}
                </p>
            </div>
        ...
        <form action="{{ route('admin.gallery-videos.update', $galleryVideo) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="title_en" id="title_en"
                           value="{{ old('title_en', $galleryVideo->getTranslation('title', 'en', false)) }}"
                ...
                <div>
                    ...
                    <input type="text" name="title_id" id="title_id"
                           value="{{ old('title_id', $galleryVideo->getTranslation('title', 'id', false)) }}"
            ...
            <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-5"
                 x-data="{
                     youtubeUrl: @js(old('youtube_url', $galleryVideo->youtube_url ?? '')),
                     videoId: @js(old('youtube_url') ? null : ($galleryVideo->youtube_id ?? null)),
                     get thumbnailUrl() {
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order"
                               value="{{ old('sort_order', $galleryVideo->sort_order) }}" min="0"
                    ...
                    <div class="flex items-start pt-7">
                        <label class="flex items-center gap-3 cursor-pointer select-none">
                            ...
                            <input type="checkbox" name="is_active" id="is_active" value="1"
                                   {{ old('is_active', $galleryVideo->is_active) ? 'checked' : '' }}
            ...
            {{-- Meta Info --}}
            <div class="bg-slate-50 rounded-2xl px-5 py-4 border border-slate-200/80 text-xs text-slate-400 space-y-1">
                <p><span class="font-semibold text-slate-500">Dibuat:</span> {{ $galleryVideo->created_at->format('d M Y, H:i') }}</p>
                <p><span class="font-semibold text-slate-500">Diperbarui:</span> {{ $galleryVideo->updated_at->format('d M Y, H:i') }}</p>
            </div>
