<x-layouts.admin title="Edit Job Posting">
    <div class="max-w-5xl mx-auto space-y-6" x-data="{
        ...
        titleEn: {{ json_encode(old('title_en', $career->getTranslation('title', 'en', false))) }},
        titleId: {{ json_encode(old('title_id', $career->getTranslation('title', 'id', false))) }},
        descEn: {{ json_encode(old('description_en', $career->getTranslation('description', 'en', false))) }},
        descId: {{ json_encode(old('description_id', $career->getTranslation('description', 'id', false))) }},
    }">
        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Job Posting</h2>
                <p class="text-sm text-slate-500 mt-1">Update the job opening details in English and Indonesian.</p>
            </div>
            <a href="{{ route('admin.careers.index') }}" class="text-sm font-semibold text-slate-600 hover:text-slate-900">← Back</a>
        </div>

        <form action="{{ route('admin.careers.update', $career) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="type" id="type"
                                ...
                                @foreach(\App\Enums\JobType::cases() as $type)
                                    <option value="{{ $type->value }}" {{ old('type', $career->type->value) === $type->value ? 'selected' : '' }}>
                                        {{ $type->label() }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if($career->image)
                            <div class="space-y-2">
                                ...
                                <img src="{{ $career->imageUrl() }}" alt="Current image" class="w-full h-32 object-cover rounded-xl border border-slate-200">
                            </div>
                        @endif

                        {{-- New Image Upload --}}
                        <div class="space-y-1.5">
                            <label for="image" class="block text-sm font-semibold text-slate-700">{{ $career->image ? 'Replace Image' : 'Cover Image' }}</label>
