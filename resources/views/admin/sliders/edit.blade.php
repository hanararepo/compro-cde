<x-layouts.admin title="Edit Slider">
    <div class="max-w-3xl mx-auto space-y-6" x-data="{
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Slider</h2>
                <p class="text-sm text-slate-500 mt-1">
                    {{ $slider->getTranslation('title', 'en', false) ?: $slider->getTranslation('title', 'id', false) ?: 'Slider' }}
                </p>
            </div>
                ...
                <div x-show="lang === 'en'" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title_en" id="title_en"
                               value="{{ old('title_en', $slider->getTranslation('title', 'en', false)) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description_en" id="description_en" rows="3"
                                  ...
                                  class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-400 bg-slate-50/50 resize-none">{{ old('description_en', $slider->getTranslation('description', 'en', false)) }}</textarea>
                ...
                <div x-show="lang === 'id'" class="space-y-4" style="display: none;">
                    <div>
                        ...
                        <input type="text" name="title_id" id="title_id"
                               value="{{ old('title_id', $slider->getTranslation('title', 'id', false)) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description_id" id="description_id" rows="3"
                                  ...
                                  class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-400 bg-slate-50/50 resize-none">{{ old('description_id', $slider->getTranslation('description', 'id', false)) }}</textarea>
                ...
                <div class="border-t border-slate-100 pt-5 space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">
                            Desktop Image
                            <span class="ml-1 text-xs font-normal text-slate-400">(leave empty to keep current image)</span>
                        </label>
                        ...
                        <div class="mb-3 flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                            <img src="{{ $slider->desktopImageUrl() }}" alt="current desktop"
                                 class="h-16 w-28 object-cover rounded-lg ring-1 ring-slate-200">
                            <div class="text-xs text-slate-500">
                                <p class="font-semibold text-slate-700">Current image</p>
                                <p class="mt-0.5">Upload a new image below to replace it.</p>
                            </div>
                        </div>
                        ...
                        <div class="relative border-2 border-dashed border-slate-200 rounded-xl overflow-hidden bg-slate-50 hover:border-brand-400 transition-colors"
                            ...
                            <template x-if="!desktopPreview">
                                <label for="desktop_input_edit" class="flex flex-col items-center justify-center py-8 cursor-pointer">
                                    ...
                                    <span class="text-sm font-semibold text-slate-500">Click or drag to replace desktop image</span>
                                </label>
                            </template>
                            <input id="desktop_input_edit" type="file" name="image_desktop" x-ref="desktopInput"
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mb-2">Recommended: 768×500px (ratio 16:10). JPG, PNG, WebP format. Max 3MB.</p>

                        {{-- Current Mobile Image --}}
                        @if($slider->image_mobile)
                            <div class="mb-3 flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200" x-data="{ removing: false }">
                                <img src="{{ $slider->mobileImageUrl() }}" alt="current mobile"
                                     class="h-16 w-10 object-cover rounded-lg ring-1 ring-slate-200">
                                <div class="flex-1 text-xs text-slate-500">
                                    <p class="font-semibold text-slate-700">Current mobile image</p>
                                    <p class="mt-0.5">Upload a new image to replace, or check to remove.</p>
                                </div>
                                <label class="flex items-center gap-1.5 cursor-pointer select-none">
                                    ...
                                    <span class="text-xs font-semibold text-rose-600">Remove</span>
                                </label>
                            </div>
                        @endif
                        ...
                        <div class="relative border-2 border-dashed border-slate-200 rounded-xl overflow-hidden bg-slate-50 hover:border-brand-400 transition-colors"
                            ...
                            <template x-if="!mobilePreview">
                                <label for="mobile_input_edit" class="flex flex-col items-center justify-center py-8 cursor-pointer">
                                    <svg class="w-8 h-8 text-slate-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                    </svg>
                                    <span class="text-sm font-semibold text-slate-500">Click or drag to {{ $slider->image_mobile ? 'replace' : 'add' }} mobile image</span>
                                    <span class="text-xs text-slate-400 mt-1">JPG, PNG, WebP • max 3MB</span>
                                </label>
                            </template>
                            <input id="mobile_input_edit" type="file" name="image_mobile" x-ref="mobileInput"
                                   accept="image/*" class="sr-only"
