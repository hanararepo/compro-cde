<x-layouts.admin title="Gallery Categories">
    <div x-data="liveTable('{{ route('admin.gallery-categories.index') }}', { search: '{{ request('search') }}' })" class="space-y-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Gallery Categories</h2>
                <p class="text-sm text-slate-500 mt-1">Organize photos and media assets into categories.</p>
            </div>
            <a href="{{ route('admin.gallery-categories.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold shadow-sm shadow-brand-600/30 transition-all">
            ...
            <div class="flex items-center gap-3">
                <div class="relative flex-1">
                    ...
                    <input 
                        ...
                        placeholder="Type to search gallery categories instantly..." 
        ...
        <!-- Live Gallery Categories Table Container -->
        <div id="live-table-container">
            <x-data-table :headers="['Category Name (EN / ID)', 'Slug', 'Images Count', 'Actions']" :paginator="$categories">
                @forelse($categories as $category)
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-6 py-4 text-xs font-semibold text-slate-700">
                            {{ $category->galleries_count }} images
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                @can('gallery-categories.edit')
                                ...
                                @can('gallery-categories.delete')
                                    <form action="{{ route('admin.gallery-categories.destroy', $category) }}" method="POST" class="inline-flex"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-sm text-slate-400">
                            No gallery categories found matching your search.
                        </td>
                    </tr>
                @endforelse
            </x-data-table>
