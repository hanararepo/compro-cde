<x-layouts.admin title="Article Tags">
    <div x-data="liveTable('{{ route('admin.article-tags.index') }}', { search: '{{ request('search') }}' })" class="space-y-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Article Tags</h2>
                <p class="text-sm text-slate-500 mt-1">Manage tags used to label and filter articles.</p>
            </div>
            @can('article-tags.create')
                <a href="{{ route('admin.article-tags.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold shadow-sm shadow-brand-600/30 transition-all">
            ...
            <div class="flex items-center gap-3">
                <div class="relative flex-1">
                    ...
                    <input
                        ...
                        placeholder="Type to search tags instantly..."
        ...
        <div id="live-table-container">
            <x-data-table :headers="['Tag Name (EN / ID)', 'Slug', 'Articles', 'Actions']" :paginator="$tags">
                @forelse($tags as $tag)
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-6 py-4">
                            <div>
                                <p class="font-bold text-slate-900 text-sm">{{ $tag->getTranslation('name', 'en') }}</p>
                                <p class="text-xs text-slate-400">ID: {{ $tag->getTranslation('name', 'id') }}</p>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                ...
                                @can('article-tags.delete')
                                    <form action="{{ route('admin.article-tags.destroy', $tag) }}" method="POST" class="inline-flex"
                                          onsubmit="return confirm('Delete tag \'{{ $tag->getTranslation('name', 'en') }}\'? This will remove it from all articles.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
